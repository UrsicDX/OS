...
int execute_external(int argc, char **argv)
{
    ...
    else if (pid == 0)
    {
        // Child process
        char *cmd = argv[0];
        char *args[argc + 1]; // Array to hold command and its arguments, plus one extra space for NULL terminator
        for (int i = 0; i < argc; ++i)
        {
            args[i] = argv[i]; // Copy command and arguments
        }
        args[argc] = NULL; // NULL terminator required by execvp

        execvp(cmd, args); // Execute command with arguments
...
int main()
{
    ...
    while (1)
    {
        ...
        if (builtin_status == -1)
        {
            last_command_status = execute_external(token_count, tokens);
        }
    }
